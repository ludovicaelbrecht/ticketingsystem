...
Partial Class verwijderArtikel
...
#End Region
    ...
    Public Sub verwijderArtikel()
        Dim strScript As String
        Dim nr As Integer

        nr = CInt(Val(Request.QueryString("artikelNr")))

        If Session.Contents("loginNaam") = "" Then
            strScript = "<script type='text/javascript'>alert('U bent niet aangemeld.\nOnaangemeld kan u deze pagina niet bezichtigen.');window.location='home.aspx';</script>"
            ' de redirect gebeurt via javascript omdat de alert voor response.redirect niet meer werkt
            RegisterClientScriptBlock("NietAangemeld", strScript)

        ElseIf CInt(Val(nr)) < 1 Then ' stel: 0 a
            strScript = "<script type='text/javascript'>alert('Er is een fout artikelnummer meegegeven.\nGelieve opnieuw te proberen.');</script>"
            RegisterClientScriptBlock("FoutNr", strScript)
        ElseIf CInt(Val(nr)) <> nr Then ' stel: 1.5 1,5 a
            strScript = "<script type='text/javascript'>alert('Er is een fout artikelnummer meegegeven.\nGelieve opnieuw te proberen.');</script>"
            RegisterClientScriptBlock("FoutArtNr", strScript)
        Else
            Session.Contents("artikelNr" & nr) = 99999
            Response.Redirect("overzichtwinkelkar.aspx")
        End If
    End Sub
